...
<div style="padding: 30px;">
	...
	<c:if test="${not empty vacAppDto}">
		<div
			style="background: white; border-radius: 10px; box-shadow: 0 1px 4px rgba(0, 0, 0, 0.08); padding: 24px; margin-bottom: 16px;">
			<h3
				style="margin: 0 0 16px 0; font-size: 15px; color: var(--main-color); font-weight: 600;">휴가
				정보</h3>
			<table
				style="width: 100%; border-collapse: collapse; font-size: 14px;">
				<tr>
	...
	<c:if test="${not empty expAppDto}">
		<div
			style="background: white; border-radius: 10px; box-shadow: 0 1px 4px rgba(0, 0, 0, 0.08); padding: 24px; margin-bottom: 16px;">
			<h3
				style="margin: 0 0 16px 0; font-size: 15px; color: var(--main-color); font-weight: 600;">품의
				정보</h3>
			<table
				style="width: 100%; border-collapse: collapse; font-size: 14px;">
				<tr style="border-bottom: 1px solid #f0f0f0;">
	...
	<c:if test="${not empty dftAppDto}">
		<div
			style="background: white; border-radius: 10px; box-shadow: 0 1px 4px rgba(0, 0, 0, 0.08); padding: 24px; margin-bottom: 16px;">
			<h3
				style="margin: 0 0 16px 0; font-size: 15px; color: var(--main-color); font-weight: 600;">업무기안
				정보</h3>
			<table
				style="width: 100%; border-collapse: collapse; font-size: 14px;">
				<tr>
		...
		<table
			...
			<thead>
				<tr style="border-bottom: 2px solid var(--main-color);">
					<th
						style="padding: 12px 16px; text-align: left; color: var(--main-color); font-weight: 600;">순서</th>
					<th
						style="padding: 12px 16px; text-align: left; color: var(--main-color); font-weight: 600;">결재자</th>
					<th
						style="padding: 12px 16px; text-align: left; color: var(--main-color); font-weight: 600;">부서</th>
					<th
						style="padding: 12px 16px; text-align: left; color: var(--main-color); font-weight: 600;">직급</th>
					<th
						style="padding: 12px 16px; text-align: center; color: var(--main-color); font-weight: 600;">상태</th>
					<th
						style="padding: 12px 16px; text-align: left; color: var(--main-color); font-weight: 600;">결재일</th>
					<th
						style="padding: 12px 16px; text-align: left; color: var(--main-color); font-weight: 600;">반려사유</th>
				</tr>
			</thead>
			<tbody>
	...
	<c:if test="${myTurn != null}">
		<div
			...
			<form action="/appr/approve" method="post"
				...
				<input type="hidden" name="appId" value="${appDto.appId}"> <input
					type="hidden" name="currentOrder" value="${myTurn.appLineOrder}">
				<button type="submit"
					style="padding: 12px 28px; background: var(--main-color); color: white; border: none; border-radius: 8px; font-size: 14px; font-weight: 600; cursor: pointer;">
					✅ 승인</button>
			</form>
			<button type="button" onclick="openRejectPopup()"
				style="padding: 12px 28px; background: #ffebee; color: #c62828; border: 1px solid #c62828; border-radius: 8px; font-size: 14px; font-weight: 600; cursor: pointer;">
				❌ 반려</button>
		</div>
	</c:if>
...
<div id="rejectPopup"
	...
	<div
		style="background: white; padding: 30px; border-radius: 12px; width: 440px; box-shadow: 0 4px 20px rgba(0, 0, 0, 0.15);">
		...
		<form action="/appr/reject" method="post"
			...
			<textarea id="rejectReason" name="appLineRej"
				placeholder="반려 사유를 입력하세요. (최대 300자)" rows="4" maxlength="300"
				style="width: 100%; padding: 12px; border: 1px solid #ddd; border-radius: 8px; font-size: 14px; resize: none; box-sizing: border-box;"></textarea>
			<div
				style="display: flex; gap: 8px; justify-content: center; margin-top: 16px;">
				<button type="submit"
					style="padding: 10px 24px; background: #ffebee; color: #c62828; border: 1px solid #c62828; border-radius: 6px; font-size: 14px; font-weight: 600; cursor: pointer;">
					반려 확정</button>
				<button type="button" onclick="closeRejectPopup()"
					style="padding: 10px 24px; background: #f0f0f0; color: #333; border: none; border-radius: 6px; font-size: 14px; cursor: pointer;">
					취소</button>
			</div>
		</form>
	</div>
