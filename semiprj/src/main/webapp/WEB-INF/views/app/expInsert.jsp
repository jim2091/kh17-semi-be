	...
	$(function() {
		var state = {
			...
			ok : function() {
				return this.appTitleValid && this.approverValid
						&& this.appContentValid && this.expPriceValid
						&& this.expDateValid && this.expHistoryValid
						&& this.expHowValid;
			}
		};
		...
		$("[name=expPrice]").on("input blur", function() {
			...
			if (numericValue <= 0) {
				...
				$group.find(".fail-feedback").text("지출 금액은 0원 이하일 수 없습니다.");

				state.expPriceValid = false;
				return;
			}

			cleanValue = String(numericValue);
		...
		window.checkApproverState = function() {
			...
			if (!approver1) {
				$group.addClass("has-error");
				$group.find(".appr-box-item .field").removeClass("success")
						.addClass("fail");
				state.approverValid = false;
			} else {
				// 결재자 유입 즉시 빨간색 경고등 스킨 강제 오프(Off) 및 원상복구
				$group.removeClass("has-error");
				$group.find(".appr-box-item .field").removeClass("fail")
						.addClass("success");
	...
	<div class="exp-container">
		...
		<form action="./expInsert" method="post" id="approvalForm"
			...
			<div class="form-group">
				<label>결재명 <span class="required">*</span></label> <input
					type="text" name="appTitle" class="field"
			...
			<div class="form-group">
				<label>결재내용 <span class="required">*</span></label> <input
					type="text" name="appContent" class="field"
					placeholder="상세 지출 내용을 명시하십시오.">
				<div class="fail-feedback">결재 상세 품의 사유를 명시해 주십시오.</div>
			</div>

			<div class="form-group">
				<label>지출금액 <span class="required">*</span></label> <input
					type="text" name="expPrice" class="field" placeholder="숫자만 입력하세요">
			...
			<div
				...
				<div class="form-group">
					<label>지출일 <span class="required">*</span></label> <input
						type="date" name="expDate" class="field">
			...
			<div class="form-group">
				<label>지출내역 <span class="required">*</span></label> <input
					type="text" name="expHistory" class="field"
			...
			<div class="form-group">
				<label>지출방법 <span class="required">*</span></label> <input
					type="text" name="expHow" class="field"
