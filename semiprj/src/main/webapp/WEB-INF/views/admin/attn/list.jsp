...
<div class="attn-content-body" style="flex-grow: 1; padding-left: 40px; font-family: 'Malgun Gothic', sans-serif;">
    ...
    <form action="/admin/attn/list" method="get" class="search-box">
    ...
    <div class="pagination">
        ...
        <c:if test="${pageVO.hasPrevious()}">
            <a href="/admin/attn/list?page=${pageVO.previousBlock}&${query}" class="page-box">◀</a>
        </c:if>
        <c:forEach var="i" begin="${pageVO.beginBlock}" end="${pageVO.endBlock}">
            <a href="/admin/attn/list?page=${i}&${query}" class="page-box ${i == pageVO.page ? 'active' : ''}">${i}</a>
        </c:forEach>
        <c:if test="${pageVO.hasNext()}">
            <a href="/admin/attn/list?page=${pageVO.nextBlock}&${query}" class="page-box">▶</a>
        </c:if>
    </div>
</div>
