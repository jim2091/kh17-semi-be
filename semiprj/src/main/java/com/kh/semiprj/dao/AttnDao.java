...
public class AttnDao {
	...
	public void updateStatusToAbsent() {
		String sql = "UPDATE attn SET attn_record = '결근' "
				   ...
				   + "  /* 🎯 [추정 안전장치] 대상일(어제)이 토요일(6)이나 일요일(7)이면 결근 처리 안 함 */ "
				   + "  AND TO_CHAR(SYSDATE - 1, 'D') NOT IN ('1', '7') "
	...
	public void createTodayAttendance() {
		String sql = "INSERT INTO attn (attn_id, emp_no, attn_work_date, attn_record) "
				   ...
				   + "  /* 🎯 [추가] 오늘이 토요일(7) 또는 일요일(1)이면 인서트 자체를 하지 않음 (오라클 기본 포맷 기준 1=일, 7=토) */ "
	...
	public void updateCheckOut(String empNo) {
	    String sql = "UPDATE attn SET " 
	            ...
	            + "  attn_work_time = CASE "
	            + "       /* 출근이나 퇴근 시간 중 하나라도 누락되면 0 처리 */ "
	            + "       WHEN attn_in_time IS NULL THEN 0 "
	            + "       "
	            + "       /* 1. 점심시간(12:00~13:00) 전에 퇴근한 경우 : 점심시간 제외하고 순수 근무시간만 계산 */ "
	            + "       WHEN SYSDATE < TRUNC(SYSDATE) + 12/24 "
	            + "       THEN GREATEST(ROUND((SYSDATE - attn_in_time) * 24, 2), 0) "
	            + "       "
	            + "       /* 2. 점심시간(13:00) 이후에 출근한 경우 : 점심시간을 겪지 않았으므로 빼지 않고 계산 */ "
	            + "       WHEN attn_in_time >= TRUNC(SYSDATE) + 13/24 "
	            + "       THEN GREATEST(ROUND((SYSDATE - attn_in_time) * 24, 2), 0) "
	            + "       "
	            + "       /* 3. 점심시간 중에 출근한 경우 : 13:00부터 근무한 것으로 계산 */ "
	            ...
	            + "       "
	            + "       /* 4. 일반적인 경우 (오전 출근 ~ 오후 퇴근) : 총 시간에서 점심시간 1시간(-1) 제외 */ "
	            + "       ELSE GREATEST(ROUND(((SYSDATE - attn_in_time) * 24) - 1, 2), 0) " 
	            + "  END, "
	            + "  attn_record = CASE " 
	            + "       /* 위에서 계산될 실제 근무시간이 8시간 미만이고 기존 상태가 '정상근무'였다면 '조퇴'로 변경 */ "
	            + "       WHEN (CASE "
